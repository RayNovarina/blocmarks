...
class UsersController < ApplicationController
  def show
    ...
    @topics_with_user_bookmarks = Topic.all.joins(:bookmarks).where('bookmarks.user_id = ?', current_user.id).reorder('lower(topics.title) ASC').select('topics.*').distinct('topics.id')
    ...
    @topics_with_user_liked_bookmarks = Topic.all.joins(bookmarks: :likes).where('likes.user_id = ?', current_user.id).reorder('lower(topics.title) ASC').select('topics.*').distinct('topics.id')
